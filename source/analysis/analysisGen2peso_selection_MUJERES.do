...
global gphoptns scheme(lean2) ///
				ytitle("Daughter's birth weight in grams (2nd generation)") ///
...
-> children born as girls: NAC_SEXO == 2
*/
gen g2fsmpl = NAC_mrg_mbdata2main == 3 ///
	& NAC_EDAD_MADRE >= 15 & NAC_EDAD_MADRE <= 45 ///
	& NAC_ANO_NAC >= 2007 & NAC_SEXO == 2

* Show birth years for cohorts in sample:
tab NAC_ANO_NAC g2fsmpl

* Show birth years for the MOTHERS in the sample:
tab NAC_ANO_NAC_MADRE g2fsmpl if birth_order_by_mother_u == 1

* Summarize dependent variable by birth year, restricted to sample:
tabstat $mydepvar if g2fsmpl == 1, by(NAC_ANO_NAC) s(N mean min max)
...
sum NAC_ANO_NAC_MADRE if g2fsmpl == 1
...
rdbwselect $mydepvar NAC_PESO_MADRE if g2fsmpl == 1 & sem32m == 1, c(1500)
...
rdplot $mydepvar NAC_PESO_MADRE ///
	if optbw_o32 == 1 & g2fsmpl == 1 & sem32m == 1, ///
	c(1500) nbins(10) p(2) ///
	graph_options(${gphoptns} name(${abbrev}_o32_optbw_10bins, replace))

* robust rd estimation:
rdrobust $mydepvar NAC_PESO_MADRE if g2fsmpl == 1 & sem32m==1, c(1500) scalepar(-1) all
...
sum $mydepvar if optbw_o32 == 1 & (g2fsmpl == 1 | app_deadmom == 1) & sem32m == 1
...
forval p = 10(10)90 {
	* graph:
	...
	if optbw_o32 == 1 & (g2fsmpl == 1 | app_deadmom == 1) & sem32m == 1, ///
	c(1500) nbins(10) p(2) ///
	graph_options(${gphoptns} name(${abbrev}i`p'_o32_optbw_10bins, replace))
	
	* robust rd estimation:
	rdrobust ${mydepvar}_imp`p' NAC_PESO_MADRE if (g2fsmpl == 1 | app_deadmom == 1) & sem32m==1 , c(1500) scalepar(-1) all
	...
	local ef = e(N_h_l)+e(N_h_r)
	estadd scalar effopt = `ef'
	sum ${mydepvar}_imp`p' if optbw_o32 == 1 & (g2fsmpl == 1 | app_deadmom == 1) & sem32m == 1 & ${mydepvar}_imp`p' != . & NAC_PESO_MADRE != . & abs(NAC_PESO_MADRE - 1500) <= e(h_l)
	estadd scalar dvmean = r(mean)
	
	count if (g2fsmpl == 1 | app_deadmom == 1) & sem32m == 1 & ${mydepvar}_imp`p' != . & NAC_PESO_MADRE != . & abs(NAC_PESO_MADRE - 1500) <= e(h_l) & NAC_PESO_MADRE < 1500 & app_deadmom == 1
...
/* 
Baseline estimates an approximately -274.84 gram effect of treatment (not 
statistically significant).

Even if the girls that died within 1 year of life had somehow survived and given
birth to girls in the 10th percentile of weight, the effect of treatment would 
still be far from zero, although statistically significant.

This is evidence of strong negative selection in the control group.
*/
