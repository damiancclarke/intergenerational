...
*--- (1) RD -- optimal
*-------------------------------------------------------------------------------
foreach year of numlist 2007 2001 2018 {
    use "$DAT/MERGED_DEF2NAC.dta", clear
    drop if mrg_DEF2NAC==2
    keep if NAC_ANO_NAC>=1992&NAC_ANO_NAC<=`year'
    hist NAC_PESO, xtitle("Birth weight (grams)") scheme(plotplain)
    graph export "$OUT/histFull`year'.eps", replace
    #delimit ;
    hist NAC_PESO if NAC_PESO>=1300&NAC_PESO<=1700,
    xtitle("Birth weight (grams)") scheme(plotplain) width(10); 
    #delimit cr
    graph export "$OUT/histShort`year'.eps", replace
    ...
    **works well with 20 bins too.  Have used 10 for now for comparison to BLN
    foreach num of numlist 1 0 {
        ...
        *local LBW = e(h_msetwo_l)
        ...
        graph export "$OUT/imrt_`weeks'32_optimal_1992`year'.eps", replace;
        rdplot muerte_1a NAC_PESO, c(1500) nbins(20) p(2)
        graph_options(scheme(lean2) ytitle("Infant Mortality") xtitle("Weight in grams")
                      legend(off) ylabel(0.05(0.05)0.2));
        graph export "$OUT/imrt_`weeks'32_optimal_1992`year'_20.eps", replace;
        rdplot muerte_1a NAC_PESO, c(1500) 
        graph_options(scheme(lean2) ytitle("Infant Mortality") xtitle("Weight in grams")
                      legend(off));
        #delimit cr
        graph export "$OUT/imrt_`weeks'32_optimal_1992`year'_optBin.eps", replace
        restore
...
gen sem32 = NAC_SEMANAS>=32 if NAC_SEMANAS!=.
*Fallecidos menores 1 año
gen dias_muerte=DEF_FECHA_DEF_SIF-NAC_FECHA_NACIMIENTO_SIF 
*Días vividos antes de morir*
gen muerte_1a=1 if dias_muerte<=365
replace muerte_1a=0 if (dias_muerte==. | dias_muerte>365) 
*Missing corresponde a nacidos que permanecen vivos*
tab muerte_1a, m 
...
foreach num of numlist 1 0 {
    ...
    graph_options(scheme(lean2) ytitle("Infant Mortality") xtitle("Weight in grams")
                  legend(off) ylabel(0.05(0.05)0.2));
    graph export "$OUT/imrt_`weeks'32_optimal_19922001_20.eps", replace;
    rdplot muerte_1a NAC_PESO, c(1500) binselect(es)
    graph_options(scheme(lean2) ytitle("Infant Mortality") xtitle("Weight in grams")
                  legend(off) ylabel(0.05(0.05)0.2));
    #delimit cr
    graph export "$OUT/imrt_`weeks'32_optimal_19922001_optBin.eps", replace
    restore
    eststo: rdrobust muerte_1a NAC_PESO if sem32==`num', c(1500) scalepar(-1)
    estadd scalar Hopt = e(h_l)
    estadd scalar Nl   = e(N_h_l)
...
foreach num of numlist `years' {
    ...
    *Descripción
    count if NAC_PESO<1500 
    count if NAC_PESO>=1400 & NAC_PESO<=1600
    count if NAC_PESO>=1400 & NAC_PESO<=1600 & NAC_SEMANAS>=32 
    ...
    *Derecho mayor o igual 32 semanas
    preserve
    drop if mrg_DEF2NAC==2
    replace muerte_1a=. if NAC_PESO==1500
	replace muerte_1a=. if NAC_PESO==1400
	replace muerte_1a=. if NAC_PESO==1600
    ...
    *Missing covariables
    ...
    tab sem32 
    tab sem32 if NAC_PESO>1400 & NAC_PESO<1600
...
*est9: optimal, Intergen sample, baseline (>=32 weeks)
*est10: optimal, 2018 sample, heaping control (>=32 weeks)
*est11: optimal, 2018 sample, baseline (<32 weeks)
*est12: optimal, 2018 sample, heaping control (<32 weeks)
*est13: BLN literal replication (all)
*est14: BLN literal replication (>=32 weeks)
*est15: BLN literal replication (<32 weeks)
*est16: BLN 2018 replication (all)
*est17: BLN 2018 replication (>=32 weeks)
*est18: BLN 2018 replication (<32 weeks)


gen RD_Estimate = .
lab var RD_Estimate "Birth weight < 1,500"

local ests est14 est15 est1 est3 est5 est7
#delimit ;
...
local ests est17 est18 est10 est11 est5 est7
#delimit ;
esttab `ests' using "$OUT/IMRgen1_2018.tex",
replace booktabs cells(b(fmt(%-9.4f) star) se(fmt(%-9.4f) par([ ]) )) label
stats(dvmean N Hopt effopt Nl Nr, fmt(%05.3f %12.0gc %5.1f %9.0gc %9.0gc %9.0gc) 
      label("\\ Mean of Dep. Var." "Observations" "Optimal Bandwidth" 
            "Effective Observations" "Observations (left)" "Observations (right)"))
