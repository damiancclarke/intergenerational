...
if  ("`covs_eval'"=="mean") {
	...
	******************************************************
	...
	if ("`hide'"==""){
		...
		if (`ci'==0) {
                    format rdplot_mean_y %04.2f
                    twoway (scatter rdplot_mean_y rdplot_mean_bin, sort msize(medlarge) ms(Oh)) ///
				(line y_plot_l x_plot_l, lcolor(black) sort lwidth(thick) lcolor(gs7) lpattern(solid) ) ///
				(line y_plot_r x_plot_r, lcolor(black) sort lwidth(thick) lcolor(gs7) lpattern(solid) ),  ///
				xline(`c', lcolor(black) lpattern(dash)) xscale(r(`x_min' `x_max'))  `graph_options'
		}
		else {
			if ("`shade'"==""){
